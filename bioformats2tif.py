...
def process(srcDir, dstDir, currentDir, fileName, keepDirectories):
	...
	for i, imp in enumerate(imps):
		...
		blank_imp.setOpenAsHyperStack(True)
	
		# Copy calibration
		blank_imp.setCalibration(imp.getCalibration())
	
		# Copy LUTs if present
		
		
		if imp.isComposite():
		    luts = imp.getLuts()
		    if luts is not None:
		        # Convert blank_imp to CompositeImage so we can set per-channel LUTs
		        comp_imp = CompositeImage(blank_imp, imp.getMode())  # Use same composite mode (e.g. COMPOSITE, COLOR, etc.)
		        for c in range(len(luts)):
		            comp_imp.setChannelLut(luts[c], c+1)  # channel indices are 1-based in ImageJ
		        comp_imp.setDisplayMode(imp.getDisplayMode())
		        blank_imp = comp_imp  # Overwrite the reference so we save the composite version
